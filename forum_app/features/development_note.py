...
class DevelopmentNoteFeature(BaseFeatureInterface):
    ...
    def feature_description(self):
        ...
        return "Enable development note module"
    ...
    def register(self):
        if self.is_registered(self.feature_name):
            return
        database_table_scripts_path = path.join(app_path, 'data', 'feature_database_scripts', 'development_note', 'tables')
        self.db.run_scripts_in_path(database_table_scripts_path)
        self.register_feature(self.feature_name, self.feature_description, __name__, 'Development')
    ...
    def add_note(self, json):
        sql = """
        INSERT INTO development_note (route, content) VALUES (%s, %s);
        """
        return self.db.execute(sql, (json['route'], json['note_content']))
